...
/**
 * Reflection-based integration for third-party AE2 addon keys (1.20.1 Forge).
 * <p>
 * - ArsEnergistique Source: {@code energy:ars_source} → SourceKey.KEY
 * - AppBot Mana: {@code energy:botania_mana} → ManaKey.KEY
...
public final class ThirdPartyIntegration {
    ...
    private static AEKey manaKey;
    private static boolean manaTried = false;
    ...
    public static AEKey getManaKey() {
        if (!isModLoaded("appbot")) return null;
        if (manaTried) return manaKey;
        manaTried = true;
        try {
            Class<?> clazz = Class.forName("appbot.ae2.ManaKey");
            Field field = clazz.getField("KEY");
            Object obj = field.get(null);
            if (obj instanceof AEKey) manaKey = (AEKey) obj;
        } catch (Throwable ignored) {}
        return manaKey;
    }

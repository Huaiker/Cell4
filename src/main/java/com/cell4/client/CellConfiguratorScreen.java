...
public class CellConfiguratorScreen extends AbstractContainerScreen<CellConfiguratorMenu> {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (!previewCell.isEmpty()) {
            ...
            List<Component> tooltipLines = new ArrayList<>(livePreview.getTooltipLines(ctx, this.minecraft.player, flag));
    ...
    private ItemStack buildLivePreviewStack(ItemStack source) {
        ...
        // Collect non-empty entries from each group's edit fields
        ...
        // Apply to NBT based on cell type
        ...
        // Blacklist is common to all cell types
        Cell4Util.setStringList(copy, NBTKeys.BLACKLIST, blacklist);

        // Apply pending custom name
    ...
    private Optional<TooltipComponent> buildCappedTooltipImage(ItemStack livePreview, int max) {
        ...
        // Truncate to first `max` entries; hasMoreContent=false so AE2 doesn't
        // add its own "..." line (we add "...还有N-6个" as a text line instead).
    ...
    private void renderMiniButton(GuiGraphics guiGraphics, int x, int y, int w, int h, Component text, int color, int mouseX, int mouseY) {
        ...
        // Hover highlight border (white)
        if (hovered) {
            guiGraphics.fill(x, y, x + w, y + 1, 0xFFFFFFFF);           // top
            guiGraphics.fill(x, y + h - 1, x + w, y + h, 0xFFFFFFFF);   // bottom
            guiGraphics.fill(x, y, x + 1, y + h, 0xFFFFFFFF);           // left
            guiGraphics.fill(x + w - 1, y, x + w, y + h, 0xFFFFFFFF);   // right
        }
